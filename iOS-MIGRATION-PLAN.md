...
1. **Info.plist** — Switched from manual plist to `GENERATE_INFOPLIST_FILE = YES` with `INFOPLIST_KEY_*` build settings:
   - `UIFileSharingEnabled = YES` — Files app browsing
   - `LSSupportsOpeningDocumentsInPlace = YES` — In-place file editing
   - `UILaunchScreen_Generation = YES` — Proper launch screen
   - `UIApplicationSceneManifest_Generation = YES` — Scene manifest for proper window sizing
   - `UIApplicationSupportsMultipleScenes` left at default (generated by Xcode)
   - `UISupportedInterfaceOrientations` — All orientations on iPad, portrait+landscape on iPhone
   - `UIApplicationSupportsIndirectInputEvents = YES`
2. **Security-scoped URLs** — `loadFile(url:)` calls `startAccessingSecurityScopedResource()` for proper sandbox access on real devices
3. **UISceneDelegate** — Restored SceneDelegate approach (iOS 26 expects scenes for full-window layout). AppDelegate returns `UISceneConfiguration` with `delegateClass = SceneDelegate.self`. SceneDelegate creates the window with `MainViewController()`.
4. **Adaptive layout** — Floating buttons: 40pt on iPad, 32pt on iPhone. Object list: popover on iPad, modal on iPhone.

**Key learnings:**
- iOS 26+ requires proper UISceneDelegate for full-screen window sizing. Direct window creation in AppDelegate (`window = UIWindow(frame: UIScreen.main.bounds)`) results in a small centered window on iOS 26+.
